    ...
    public void execute(CommandSender sender, String[] args) {
        if (args.length == 3) {
            ...
            this.plugin.getExecutor().execute(() -> {
                ...
                switch (result) {
                    case SUCCESS:
                        sender.sendMessage(this.plugin.getLang().getPrefixedMessage("balanceTakeOthers", args[1], this.plugin.getLang().formatCurrency(value), this.plugin.getConfig().getString("Currency.Name-Plural")));
                        break;
                    case UNKNOWN_PLAYER:
