...
public class SetCommand implements IGeldCommand {

    private Geld plugin;

    // Constructor
    public SetCommand(Geld plugin){
        this.plugin = plugin;
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        if(args.length == 3){
            ...
            if(!sender.hasPermission("money.set.others")){
            ...
            this.plugin.getExecutor().execute(() -> {
                ...
                final MoneyManager.Result result = this.plugin.getEconomy().setBalance(target.get(), value);

                switch (result){
                    case SUCCESS: sender.sendMessage(this.plugin.getLang().getPrefixedMessage("balanceSetOthers", args[1], this.plugin.getLang().formatCurrency(value), this.plugin.getConfig().getString("Currency.Name-Plural"))); break;
    ...
    public void executePlayer(Player sender, String[] args) {
        ...
        if(args.length == 3){
            ...
            this.plugin.getExecutor().execute(() -> {
                ...
                this.plugin.getEconomy().setBalance(sender.getUniqueId(), value);
                sender.sendMessage(this.plugin.getLang().getPrefixedMessage("balanceSet", this.plugin.getLang().formatCurrency(value), this.plugin.getConfig().get("Currency.Name-Plural")));
            });
        } else
